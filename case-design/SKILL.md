...
- 输出：结构化需求元素清单（每条需求一张 8 槽位卡片，有值或 N/A，不留空）
- 完成标志：每条需求卡片 8 类槽位填满；发现缺口不补齐，回溯 P1

### ③ P3 需求对象化

把结构化元素转成统一领域对象。

- 读 [需求对象化](p3-requirement-object-modeling.md)（待编写）
- 输入：结构化需求元素
...
- 读 [测试点提取](p4-test-point-extraction.md)（待编写）
...
- 读 [测试设计映射](p5-test-design-mapping.md)（待编写）
- 输入：测试点集合
...
- 读 [测试场景构建](p6-test-scenario-construction.md)（待编写）
- 输入：测试设计结果
...
- 读 [测试用例生成](p7-test-case-generation.md)（待编写）
- 输入：场景集合
