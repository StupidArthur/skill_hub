---
name: ppt-skill
description: 生成横向翻页网页 PPT（单 HTML 文件）。提供两种风格：① "电子杂志 × 电子墨水" ② "瑞士国际主义"。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"时使用。任何涉及制作分享 / 演讲 / 发布会风格网页 PPT 的工作都应调用。
---
...
>
> 本 skill 为歸藏原作的衍生作品，沿用 [AGPL-3.0 协议](./LICENSE)；衍生修改与归属声明见 [NOTICE](./NOTICE)。

## 全局硬规则 · 规则冲突时必须问用户,不自决

**当两条 skill 内部规则产生冲突时,不得自决,必须询问用户。** 默认选一个会偏离用户意图,返工代价远高于多问一次。

### 典型冲突场景

- "单 accent 规则" vs "多工具推荐"(风格 B 经常出现:多个工具都想高亮)
- "版式多样性" vs "内容密度"(7/9/11 项的尴尬数量:选哪个版式都别扭)
- "视觉简洁" vs "信息完整"(工具描述是否含链接/特点/价格)
- "中文标题 ≤ 5 字" vs "标题必须保留原意"(砍字会丢语义)
- "图片只允许标准比例" vs "用户原图比例奇葩"(重生成 vs 保真)

### 决策框架

1. **意识冲突存在** —— 不跳过,不默认选一个
2. **分析各方保护的价值** —— 说清楚每条规则为什么存在
3. **向用户呈现选项**(见下模板)
4. **等用户回复,再继续**
...
```
检测到规则冲突:[简述冲突内容,例如"单 accent 规则 vs 3 个工具都想高亮"]
...
- 规则冲突的提问,**尽量在 Phase 1 大纲阶段问完**——用户评审 `PPT分页和文本内容审核.md` 时一并决策
- Phase 2 格式化阶段不再询问(进入机械翻译)
- 例外: 大纲没覆盖到的冲突,在 Phase 2 中断也要停下来问,不要自决硬走
...
# Motion One 本地副本（离线兜底,两个风格共用,约 64KB）
...
### Step 2.5 · 输出分页大纲并暂停（显式 Phase 1 / Phase 2 分界）
...
| 阶段 | 范围 | 可否再思考 | 输出物 |
|------|------|-----------|--------|
| **Phase 1 · 规划** | 拆页 / 取舍 / 改写 / 字段标注 | ✅ 自由 | `PPT分页和文本内容审核.md` |
| **Phase 2 · 格式化** | 大纲 → HTML 的逐字段翻译 | ❌ 不可再创作 | `display-N.html` |

**Phase 2 是机械翻译,不是再思考。** 用户在大纲里改的任何字,必须 1:1 反映到 HTML;HTML 里出现的每段文字必须能在 outline 里找到出处。Phase 2 允许微调 inline style（字号 `font-size` / 间距 `gap` / 高度 `height`）以适配版式,但**不得重写或新增字段内容**。

#### 大纲 Schema（必填字段）

把规划结果写到项目根的 `PPT分页和文本内容审核.md`,格式如下:
...
- `title`: 衬线大标题(风格 A)或无衬线极细大标题(风格 B), ≤ 5 字不加 nowrap
...
回复 "approved" / "go" / "继续",或提供修改后的大纲文件路径,即可进入 Phase 2。
```
...
满足任一条件,才允许进入 Step 3 写 HTML。**未获批准前不得生成任何 slide HTML。**
...
以下资源文件不属于某个 Step，被多个 Step 引用：

| 资源 | 性质 | 被哪些 Step 引用 |
|------|------|-----------------|
| [references/themes.md](./references/themes.md) | 主题色（风格 A · 5 套） | Step 1（选主题色）/ Step 2.2（落地 :root） |
| [references/themes-swiss.md](./references/themes-swiss.md) | 主题色（风格 B · 4 套） | Step 1 / Step 2.2 |
| [references/layouts.md](./references/layouts.md) | 版式库（风格 A · 10 种） | Step 3（挑布局） |
| [references/swiss-layout-lock.md](./references/swiss-layout-lock.md) | 版式锁（风格 B · 22P 登记） | Step 3 |
| [references/layouts-swiss.md](./references/layouts-swiss.md) | 版式库（风格 B · S01-S22） | Step 3 |
| [references/components.md](./references/components.md) | 组件手册（字体/色/网格/动效） | Step 3 / Step 6（迭代） |
| [references/checklist.md](./references/checklist.md) | 踩坑清单（P0-P3） | Step 4（自检） |
| [references/screenshot-framing.md](./references/screenshot-framing.md) | 截图适配（CleanShot X 式） | Step 1（截图约定）/ Step 3 |
| [references/image-prompts.md](./references/image-prompts.md) | GPT-M 2.0 配图 prompt | Step 1（配图约定）/ Step 3 |
| [references/swiss-map-component.md](./references/swiss-map-component.md) | 瑞士风地图组件（S08 扩展） | Step 3 |
...
8. 生成后先运行校验器,再读 `checklist.md` 自检:
   - 风格 A → `node scripts/validate-magazine-deck.mjs path/to/index.html`
   - 风格 B → `node scripts/validate-swiss-deck.mjs path/to/index.html`
