...
每一页都是一个 `<section class="slide ...">`。必须带 `light` 或 `dark` 类，JS 翻页时据此切换背景（`data-theme` 可选，写了优先用，不写则从 class 推断）。

```html
<section class="slide light" data-theme="light">   <!-- 浅色页（data-theme 可选） -->
<section class="slide dark" data-theme="dark">     <!-- 深色页（data-theme 可选） -->
