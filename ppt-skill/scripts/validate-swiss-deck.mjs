...
const MAGAZINE_ONLY_CLASSES = new Set([
  'big-num', 'body-serif', 'body-zh', 'display', 'display-zh', 'frame-cap',
  'ghost', 'grid-9', 'h-hero-en', 'h-xl-en', 'h1-zh', 'h2-zh', 'h3-zh',
  'img-slot', 'mid-num', 'pillar', 'plat', 'q-big', 'rowline', 'sign',
  'split-55', 'stat',
]);

const allUsedClasses = new Set();
const classAttrReB = /class="([^"]*)"/g;
let cmB;
while ((cmB = classAttrReB.exec(htmlForSlides)) !== null) {
  cmB[1].split(/\s+/).forEach((c) => {
    if (c && !c.startsWith('data-')) allUsedClasses.add(c);
  });
}

const magazineContamination = [...allUsedClasses].filter((c) => MAGAZINE_ONLY_CLASSES.has(c));
if (magazineContamination.length) {
  errors.push(`Magazine-only class(es) detected in a Swiss (Style B) deck: ${magazineContamination.slice(0, 8).map((c) => `.${c}`).join(', ')}. Style A and Style B cannot be mixed; these classes only exist in template.html.`);
}
