...
CLI 和 B/S 后端需要与用户沟通 Go 还是 Python，选型维度见各场景的 stack.md。GUI 不改变 Wails + Go 主体，只评估是否增加 Python 执行组件。
...
- [GUI 技术栈](gui-tool/stack.md) — Wails v2 + Go 主体、Python 组件选型边界、禁用项、环境要求
- [Wails 后端模式](gui-tool/wails-backend.md) — Go 分层、绑定、测试、前端衔接
- [Python Worker 与本地服务](gui-tool/python-worker.md) — Go 管理 Python 的方式、IPC、生命周期、打包与验收
- [GUI 质量标准](gui-tool/quality.md) — 构建、测试、依赖与交付清单
...
## 核心原则
1. **选型先行**：先判断类型、评估执行层，再写代码
2. **前端共享**：带界面场景共用一套 React + Shadcn + Notion 规范
3. **Go 统一控制**：GUI 的系统能力、生命周期、配置和服务编排由 Go 负责
4. **Python 按需引入**：仅在生态能力、成熟度或隔离收益明确时增加 Python 组件
5. **后端分层**：业务逻辑与框架解耦，可独立测试
6. **质量达标**：构建、测试、依赖、进程清理和离线分发均须通过检查

## 强制约束

此技能为强制标准。一旦用户选择调用本技能：

- GUI 的桌面壳和前端桥接必须使用 Wails + Go；前端必须遵循 React + Shadcn + Notion 风规范
